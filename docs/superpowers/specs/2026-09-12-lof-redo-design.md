...
- All 49 countries needed for the crisis-label rule (§4) are present in the panel — verified directly against the raw file.
...
7. **Crisis label (built last, never upstream of steps 1–6):** `crisis_label = 1` iff `(economy, year)` matches one of:
   ...
   - Russian Crisis: `RUS` × `{1998}`
   - Argentine Crisis: `ARG` × `{2001, 2002}`
   - Global Financial Crisis: all 49 economies × `{2008, 2009}`
   - European Debt Crisis: `GRC, PRT, IRL, ESP, ITA` × `{2010, 2011, 2012}`
   - COVID-19: all 49 economies × `{2020}`
